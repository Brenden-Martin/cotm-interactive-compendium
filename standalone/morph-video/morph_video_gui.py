"""Small Windows-friendly GUI for the COTM video morph renderer."""
...
class MorphVideoApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("COTM · Video Boundary Morph")
    ...
    def _build_ui(self) -> None:
        ...
        self.scale_control = self._slider_row(controls, 5, "Resolution scale", self.scale_var, .05, 1, .05, lambda value: f"{float(value)*100:.0f}%")
        self.rate_control = self._slider_row(controls, 6, "Morph rate", self.rate_var, .15, 3, .05, lambda value: f"{float(value):.2f}×")
        self.wildness_control = self._slider_row(controls, 7, "Wildness", self.wildness_var, 0, 1, .01, lambda value: f"{float(value)*100:.0f}%")

        ttk.Label(controls, text="Steps / frame").grid(row=8, column=0, sticky="w", pady=5)
        ttk.Spinbox(controls, from_=1, to=50, textvariable=self.steps_var, width=8).grid(row=8, column=1, sticky="w", pady=5)
        ttk.Label(controls, text="More evolution, proportionally slower").grid(row=8, column=2, sticky="e", pady=5)

        ttk.Label(controls, text="Random seed").grid(row=9, column=0, sticky="w", pady=5)
        ttk.Entry(controls, textvariable=self.seed_var).grid(row=9, column=1, sticky="ew", pady=5)
        ttk.Label(controls, text="Blank = new run").grid(row=9, column=2, sticky="e", pady=5)

        options = ttk.Frame(controls, style="Card.TFrame")
        options.grid(row=10, column=0, columnspan=3, sticky="w", pady=(12, 4))
        ...
        note = (
            "Full bank mode chooses among all unique presets and applies one of six RGB permutations at every new anchor. "
            "100% scale follows the original dimensions and may render far below real time."
        )
        ttk.Label(controls, text=note, wraplength=520, justify="left").grid(row=11, column=0, columnspan=3, sticky="ew", pady=(10, 0))
    ...
    def _choose_input(self) -> None:
        ...
        if not self.output_var.get():
            source = Path(path)
            self.output_var.set(str(source.with_name(f"{source.stem}-cotm-morph.mp4")))
