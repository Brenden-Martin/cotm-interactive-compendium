"""Offline video-boundary renderer for the COTM nonlinear DEQ morph bank.
...
def frame_to_boundary(frame: np.ndarray, width: int, height: int) -> np.ndarray:
...
def _remux_audio(video_path: Path, source_path: Path, output_path: Path) -> tuple[bool, str | None]:
    muxed_path = _temporary_video_path(output_path)
    command = [
        imageio_ffmpeg.get_ffmpeg_exe(), "-nostdin", "-y", "-loglevel", "error",
        "-i", str(video_path), "-i", str(source_path),
        "-map", "0:v:0", "-map", "1:a?", "-c:v", "copy", "-c:a", "aac", "-shortest", str(muxed_path),
    ]
...
def render_video(
    ...
    *,
    ...
    total_frames = int(round(duration * source_fps)) if duration > 0 and math.isfinite(duration) else None
    ...
    try:
        for frame_bytes in reader:
            ...
            weights = frame_to_boundary(frame, width, height)
    ...
    if options.preserve_audio:
        audio_preserved, warning = _remux_audio(temporary_path, source_path, destination_path)
    else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render a COTM nonlinear DEQ field using video RGB boundary weights.")
    ...
    parser.add_argument("--steps-per-frame", type=int, default=1, help="Simulation steps per source frame")
