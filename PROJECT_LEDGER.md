...
- **MUSIC-009 — Testing — Audio-linked Glitch Goo monitor.** The shared listening room places a pixel-preserving three-channel field inside a mock CRT with reflecting wall conditions. A visibly live scrolling spectrogram feeds and overlays the field; the visitor can choose 32–256 Fourier bins and an 8–60 Hz spectrum refresh rate. The three computer controls randomize across the preserved 280-preset bank, capture the current artwork-plus-spectrum boundary, and clear boundary forcing. Amplitude still controls wildness while low, middle, and high summaries independently force RGB behavior after play.
