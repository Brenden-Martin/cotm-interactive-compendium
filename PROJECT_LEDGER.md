...
- **GOO-003 — Review — Coupling equalizer and waveform drive.** Give the low, mid, two transition, high, and overall audio channels independent 0–3× coupling trims with conservative sub-unity defaults so ordinary music retains headroom instead of pinning every morph parameter. Both the Gallery instrument and Sold As Is rooms can switch between smoothed RMS envelopes and true parallel band-filtered time-domain waveforms; signed waveform values can reverse rotation and hue travel while the same shared filter-bank implementation keeps both applications aligned.
