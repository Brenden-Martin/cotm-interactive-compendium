...
The museum checkout contains thirty-six gallery exhibits; Fractal Vines, Phase March, and Cellular Automata remain private-workbench-only pending hands-on review:
...
- **VINES-001 — Review:** Fractal Vines grows pointer-plantable spline lineages with probabilistic splitting and leaf termination, variable child counts, saturated logarithmic age-thickening, smooth correlated curvature, live classic-tree-to-stochastic-vine chaos, straight-to-fan-to-radial directional spread, deterministic cuttings, and a horizontally advected coherent wind field with independent parented branch and leaf responses. Tip-hinged leaves grow to maturity; five-petal flowers usually bud over an existing leaf, open from an overlapped green bud, and traverse toward their chosen pigment. Each randomized lineage or bouquet flips an explicit flower-enable coin. Broad painted sunlight now supplies a gentle directional suggestion while shade repels. Ordinary growth defaults to a 300-step lineage ceiling, bouquets have an independent hard 100-step ceiling, and all modes stop at a shared maximum of 500 leaves plus flowers. Continuous near-tip decimation draws every sample at least once and collapses only a recent leafless, non-fork predecessor; settled geometry is never revisited. Full scrambles can remain unbounded or mutate repeatedly around a locked parent cutting, and a separate tab renders the deterministic geometric-tree family.
- **PHASE-001 — Review:** Phase March separates a touch-paintable, genuinely unwrapped phase boundary from the cyclic pigment used to display it. Gaussian brush turns, five cyclic color maps, signed time direction, phase magnification, correlated disorder, linear, concentric, spiral, and saddle presets, photo-luminance phase import with live photo hue cycling, three lattice sizes, and optional finite-gradient contour-velocity arrows expose how identical uncoupled local clocks organize into apparent traveling waves along a static spatial phase gradient.
- **CELL-001 — Review:** Cellular Automata begins with Brenden's childhood collision-fractal chamber: one-shot newborn emitters, exact diagonal Sierpinski and orthogonal Celtic-cross stencils, default pair-annihilation parity, optional annihilate-all collisions, touch-editable symmetric seeds, a complete 5×5 offspring rule editor, and a separate multistate color-reaction laboratory. The original 101×73 field remains the default at unchanged detail, while selectable 2×, 4×, 8×, 16×, and 32× linear fields expose long-term growth; randomized stencils begin at the center because their propagation direction is unknown. Wolfram rules, Conway Life, and a Turing-machine interpreter remain deliberately reserved as later additive tabs.
...
- **PHASE-001 — Phase March.** Added Gallery exhibit 35 as a stacked phase laboratory: the upper canvas edits an unwrapped scalar phase through pressure-aware Gaussian turns while the lower canvas applies one shared harmonic clock and cyclic pigment, making wavefront travel emerge from the stored gradient alone. Six generated boundary fields, five cyclic palettes, signed marching, phase magnification, optional phase-velocity arrows, correlated randomization, photo-derived phase, and original-photo hue cycling remain live and mobile-playable. This checkpoint is private-workbench-only pending hands-on review.
- **VINES-002 / PHASE-002 — Gallery doorway containment.** Restored the missing positioning context on the two newest Gallery cards so their full-card procedural artwork remains clipped behind its own title instead of escaping upward over the Gallery page.
- **VINES-003 / VINES-004 — Horticultural dynamics, continuous topology, and geometric trees.** Re-anchored every leaf at its stem-facing tip, added gradual leaf growth and five-petal flowers that predominantly share leaf nodes, and exposed bloom timing, opening, pigment, and attachment probability. Every randomized lineage and bouquet now flips a fair flower-enable coin. Bouquet mode derives three stable species from the current or locked parent using the exact Mutation Distance setting, rolls a reproducible mixture, and plants ten to eighteen sprouts whose branching, curvature, leaf, flower, scale, and pigment traits remain distinct under the shared live climate. Replaced batch threshold pruning and the retained-node ceiling with logical maximum depth plus continuous local decimation: every new segment reaches a rendered frame, then only its recent predecessor may be bypassed when that predecessor is not a root, fork, leaf, or flower. Mature geometry is never scanned or altered, and retired samples leave the live render set immediately. Retuned sunlight into a larger, blurrier, much gentler steering field. Added a separate deterministic geometric-tree tab with child-count, angle, length-ratio, recursion, trunk, twig, leaf, pigment, and reveal controls. Focused engine tests cover field polarity, near-tip eligibility, protected samples, and eased growth. This checkpoint remains private-workbench-only pending hands-on review.
- **CELL-001 — Childhood collision automata.** Added Gallery exhibit 36 around the exact elementary-school discovery: persistent cells emit a selected 5×5 stencil only once on the iteration after their birth, then overlapping offspring either cancel pairwise by parity or disappear under the optional annihilate-all rule. Exact Sierpinski and Celtic-cross presets, pointer-painted initial lattices, single/pair/fourfold/ring seed symmetries, adjustable clock, step controls, a multistate color-reaction tab, explicit annihilation cells, palette editing, collision telemetry, and a linked first-person Compendium log make the mechanism inspectable. Focused engine tests cover one-shot spawning, paired and odd collisions, both canonical stencils, arbitrary color reactions, explicit annihilation, and symmetric seeds. This checkpoint is private-workbench-only; future Wolfram, Conway, and Turing chambers remain additive work rather than placeholders masquerading as finished exhibits.
- **VINES-005 / CELL-002 — Bounded growth and deep-field observation.** Retuned Fractal Vines so ordinary lineages default to 300 steps, every three-species bouquet carries its own immutable 100-step ceiling, and the combined leaf-and-flower population can never exceed 500 before active growth stops. Cellular Automata retains its original 101×73 field pixel-for-pixel at 1× while adding 2× through 32× linearly expanded sparse lattices; the field readout reports exact dimensions, scale changes reset safely, and random 5×5 seed crystals now begin at the true center. Focused tests cover both lineage ceilings, the shared ornament limit, every field scale, original-dimension preservation, and centered random seeding. This checkpoint remains private-workbench-only pending hands-on review.
