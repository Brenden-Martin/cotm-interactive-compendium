...
import {
  ...
  DEFAULT_PALETTES, DEFAULT_ROUTES, melodyCandidates, midiToFrequency, nearestScaleIndex,
  scalePool, SECTION_CENTERS, SeededRandom,
  type EngineControls, type HarmonicEvent, type HarmonicNode, type MelodyCandidate,
  ...
  return <main className={`topo-page${controlsHidden ? " controls-hidden" : ""}`}>
    ...
    <section className="topo-console">
      ...
      <aside className="topo-controls" aria-label="Harmonic Topography controls">
        ...
        <details><summary>Machine state / portability</summary><div className="topo-transport"><button onClick={() => void copyPreset()}>Copy JSON</button><label className="topo-import"><input type="file" accept="application/json" onChange={e => { const file = e.target.files?.[0]; if (file) void importPreset(file); }} /><b>Import JSON</b></label><button onClick={() => { setBpm(58.11); setSwing(.075); setSeed(1513); setEngineControls(DEFAULT_ENGINE); setSynth(DEFAULT_SYNTH); setTerrain(DEFAULT_TERRAIN); setMix(DEFAULT_MIX); setNodes(createNodes()); setRoutes(cloneRoutes()); setPalettes(clonePalettes()); resetEngine(1513); }}>Reference preset</button></div><pre className="topo-json">{JSON.stringify({ section: active?.event.node.section ?? "INTRO", chord: active?.event.node.label ?? "Fm9", scale: active?.event.scale ?? "F Dorian", seed, fixedEventBeats: 4 }, null, 2)}</pre></details>
      </aside>
    </section>
